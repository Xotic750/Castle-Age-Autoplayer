...
ss = {
    ...
    setItem: function (name, value, hpack, compress) {
        try {
            ...
            /*jslint sub: true */
            if ($u.is_html5_sessionStorage && !ss.fireFoxUseGM) {
    ...
    getItem: function (name, value, hidden) {
        try {
            ...
            if (jsObj === undefined || jsObj === null) {
                if (!hidden) {
                    $u.warn("ss.getItem parsed string returned 'undefined' or 'null' for ", name);
                }

                if (value !== undefined && value !== null) {
                    hidden = (typeof hidden !== 'boolean') ? false : hidden;
                    if (!hidden) {
                        $u.warn("ss.getItem using default value ", value);
                    }
    ...
    used: function () {
        try {
            if ($u.is_html5_sessionStorage && !ss.fireFoxUseGM) {
                ...
                $u.log(1, "CAAP sessionStorage used: " + caapPerc + "%");
                totalPerc = ((chars * 2.048 / 5242880) * 100).dp();
                if (totalPerc >= 90) {
                    $u.warn("Total sessionStorage used: " + totalPerc + "%");
                    ...
                    window.setTimeout(function () {
                        $u.alert(message, "sessionStorage");
                    }, 5000);
                } else {
                    $u.log(1, "Total sessionStorage used: " + totalPerc + "%");
                }
            }
