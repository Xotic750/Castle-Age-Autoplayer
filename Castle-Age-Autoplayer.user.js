			...
			if (monstType.indexOf('Raid')>=0) {
				...
				miss = $("img[src*="+boss.siege_img+"]").parent().parent().text().replace(/.*:\s*Need (\d+) more to launch/, "$1").trim();
                                ...
                                if (divSeigeLogs) {
                                        ...
                                        currentPhase = divSeigeLogs - ((divSeigeLogs / 4) * 3) + 1;
                                } else {
				...
				//phaseText=Math.min(parseInt(currentPhase.attr('src').replace(/.*(\d+).jpg/, "$1"),10),boss.siege)+"/"+boss.siege+ " need " + (isNaN(+miss) ? 0 : miss);
			} else {
