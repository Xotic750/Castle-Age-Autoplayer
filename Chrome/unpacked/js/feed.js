...
/*
    ...
    feed.deleteItem = function(index) {
		try {
			if (!$u.isString(index) || index === '') {
				throw "Index supplied is 'undefined' or 'null'!";
			}
...
*/
	/*
    feed.checked = function(currentMonster) {
		try {
...
*/
    feed.items = function(type, slice) {
		try {
    ...
    feed.publicItems = function(slice) {
		try {
			slice = $u.setContent(slice, $j("#app_body"));
    ...
    feed.scanRecord = {};
/*
    feed.setScanRecord = function(md5) {
		try {
    ...
    feed.scan = function() {
		try {
			var slice = $j("#app_body"),
				...
				reviewInterval = Math.max(config.getItem("feedMonsterReviewHrs", 6), 1) * 3600,
				attackReady = false;
			...
			for (var i = 0; i < monster.records.length; i += 1) {
				...
				if (!cM.hide && cM.status == 'Join' && schedule.since(cM.review, attackReady && !tR.conditions.regex(/:burn\b/) ? 5 * 60 : reviewInterval)) {
    ...
    feed.scoring = function(cM) {
		try {
			var temp,
				...
				achleft = Math.min(0, monster.getInfo(cM.monster, 'achNum') - killed);
...
/*              htmlCode += caap.makeCheckTR('Do In Background', 'useAjaxMonsterFinder', true, "Check Monsters using AJAX rather than page navigation.");
			...
			htmlCode += caap.display.start('guildMonsterFinder');
...
*/             htmlCode += caap.makeCheckTR("Enable Tier 1", 'publicMonsterFinder1', false, "Find monsters in the Public Tier 1 Feed.");
			...
			htmlCode += caap.display.start('publicMonsterFinder2');
			...
			htmlCode += caap.display.end('publicMonsterFinder2');
