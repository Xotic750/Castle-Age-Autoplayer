...
spreadsheet = {
			...
			if(spreadsheet.records === 'default' || !$j.isArray(spreadsheet.records) || !spreadsheet.records.length) {
				...
				$j.ajax({
					...
					success : function(msg) {
						try {
							con.log(3, "msg", msg);
							var rows = [], row = 0, rowsLen = 0, column = 0, newRecord = {}, cell = null, headers = {}, headersLen = 0, headersArr = [], key = '';
	...
	getItem : function(name, image) {
		try {
			...
			var it = 0, len = 0, found = false, record = {};
	...
	getTitle : function(title, image) {
		try {
			var it = 0, tempIt = -1, owned = 0, titleStr = '', hide = false, opacity = false;
		...
		try {
			...
			if($u.hasContent(images)) {
				images.each(function() {
					var img = $j(this), div = $j(), title = '', image = '', style = '', tMes = {};
